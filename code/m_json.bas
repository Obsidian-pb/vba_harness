...
'sss "Как можно изменить код класса clsTool_ReplaceCode, так чтобы он позволял не просто заменять код но и вставлять его в произвольную часть кода. Исходяный коди при этом, если он есть после вставленнйо строки оставался нетронутым. - т.е. если вставляется несколько строк, огни не должны переписывать ранее имевшийся код!"
'sss "Напиши новый инструмент для проекта NewProj, позволяющий вставлять прпограммный код VBA в произвольную часть кода. Исходяный код при этом, если он есть после вставленнйо строки оставался нетронутым. - т.е. если вставляется несколько строк, огни не должны переписывать ранее имевшийся код!"
